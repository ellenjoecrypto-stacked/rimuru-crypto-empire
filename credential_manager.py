...
class CredentialManager:
    ...
    def _setup_encryption(self, password: str) -> bool:
        ...
        try:
            ...
            print(f"[ERROR] Error setting up encryption: {e}")
            return False
    ...
    def _load_encryption(self, password: str) -> bool:
        ...
        try:
            ...
            print(f"[ERROR] Error loading encryption: {e}")
            return False
    ...
    def add_credential(self, name: str, cred_type: str, value: str, 
        ...
        try:
            ...
            if any(c.name.lower() == name.lower() for c in self.credentials):
                print(f"[ERROR] Credential '{name}' already exists!")
                return False
            ...
            print(f"[OK] Added credential: {name}")
            return True
        except Exception as e:
            print(f"[ERROR] Error adding credential: {e}")
            return False
    ...
    def save_vault(self) -> bool:
        ...
        try:
            ...
            print(f"[OK] Vault saved ({len(self.credentials)} credentials)")
            return True
        except Exception as e:
            print(f"[ERROR] Error saving vault: {e}")
            return False
    ...
    def load_vault(self) -> bool:
        ...
        try:
            if not self.vault_file.exists():
                print("[INFO] No vault file found. Starting fresh.")
                return True
            ...
            print(f"[OK] Vault loaded ({len(self.credentials)} credentials)")
            return True
        except Exception as e:
            print(f"[ERROR] Error loading vault: {e}")
            return False
    
    def list_credentials(self, show_values: bool = False) -> None:
        """List all credentials"""
        if not self.credentials:
            print("[INFO] No credentials stored yet.")
            return
        
        print(f"\n{'='*80}")
        print(f"CREDENTIALS LIST ({len(self.credentials)} total)")
        print(f"{'='*80}\n")
    ...
    def delete_credential(self, name: str) -> bool:
        ...
        for i, cred in enumerate(self.credentials):
            if cred.name.lower() == name.lower():
                ...
                print(f"[OK] Deleted credential: {name}")
                return True
    ...
    def update_credential(self, name: str, value: str) -> bool:
        ...
        for cred in self.credentials:
            if cred.name.lower() == name.lower():
                ...
                print(f"[OK] Updated credential: {name}")
                return True
    ...
    def main_menu(self):
        ...
        while True:
            ...
            if choice == '1':
                ...
                print("\n[INPUT] Enter new value (will not be echoed):")
...
def main():
    ...
    print("  Secure Encrypted Storage for API Keys, Wallets & Secrets")
    print("="*80 + "\n")
    ...
    if manager.vault_file.exists():
        print("[INFO] Vault file found. Loading existing vault...\n")
        attempts = 0
        while attempts < 3:
        ...
        print("[INFO] Creating new vault with master password encryption...\n")
        while True:
            ...
            if manager._setup_encryption(password):
                manager.save_vault()
                print("[OK] Vault created successfully!\n")
                break
