...
print("=" * 60)
print("EDGE BROWSER WALLET DATA CHECK")
print("=" * 60)
                ...
                if os.path.exists(manifest_path):
                    try:
                        ...
                        if any(kw in name + desc for kw in ['metamask', 'wallet', 'crypto', 'coinbase', 'phantom', 'trust', 'web3']):
                            print(f"  FOUND: {manifest.get('name', '?')} v{manifest.get('version', '?')}")
                            print(f"    ID: {ext_id}")
                            print(f"    Desc: {manifest.get('description', '?')[:100]}")
                    except:
                        pass
...
for db_path in mm_db_paths:
    if os.path.exists(db_path):
        ...
        print(f"\n  DB: {os.path.basename(os.path.dirname(db_path))}")
        ...
        # Try to read LOG file for metadata
            ...
            if f.endswith('.ldb') or f.endswith('.log'):
                ...
                try:
                    ...
                    if 'vault' in text.lower():
                        print(f"    {f}: Contains vault reference ({len(data)} bytes)")
                        # Find vault JSON
            ...
            if f.endswith('.ldb') or f.endswith('.log'):
                ...
                try:
                    ...
                    if any(kw in text.lower() for kw in ['token', 'session', 'auth', 'user', 'email', 'account']):
                        ...
                        print(f"    {f}: Has session data ({len(data)} bytes)")
                        
                        # Look for email
                        import re
                        emails = re.findall(r'[\w.+-]+@[\w-]+\.[\w.]+', text)
                        if emails:
                            print(f"      Email found: {list(set(emails))[:3]}")
                except:
                    pass
...
if os.path.exists(login_db):
    try:
        ...
        if relevant:
            print(f"  Found {len(relevant)} relevant saved logins:")
            ...
            print(f"  No exchange/wallet logins saved (total logins: {len(all_logins)})")
            # Show all sites
            if all_logins:
                print("  All saved login sites:")
