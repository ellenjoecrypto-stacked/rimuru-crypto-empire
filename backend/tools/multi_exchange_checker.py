...
def check_kraken_balance():
    ...
    if not api_key or not api_secret or len(api_secret) < 30:
        logger.info("Kraken: API keys not properly configured")
        return None
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Kraken: API secret format invalid - {e}")
            return None
        ...
        for asset, amount in balances.items():
            ...
            if amount > 0.000001:
                ...
                total_usd += usd_val
                logger.info(f"  {standard}: {amount:.6f} (~${usd_val:,.2f})")
...
def check_coinbase_cdp():
    ...
    if not os.path.exists(cdp_key_path):
        logger.info("Coinbase CDP: Key file not found")
        return None
    ...
    try:
        ...
        logger.error(f"Coinbase CDP error: {e}")
        return None
        ...
        try:
            ...
            if data['status'] == '1':
                ...
                if eth > 0.0001:
                    ...
                    total_eth += eth
                    total_usd += usd
                    logger.info(f"  {addr[:10]}...{addr[-6:]}: {eth:.4f} ETH (~${usd:,.2f})")

            time.sleep(0.25)  # Rate limit

        except Exception as e:
            logger.error(f"  Error checking {addr[:10]}: {e}")
...
def main():
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"GRAND TOTAL: ${total:,.2f}")
    logger.info("=" * 60)
